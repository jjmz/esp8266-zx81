...
int ICACHE_FLASH_ATTR Z80Interrupt (Z80_STATE *state, int data_on_bus)
{
        state->status = 0;
        if (state->iff1) {
...
int ICACHE_FLASH_ATTR Z80NonMaskableInterrupt (Z80_STATE *state)
{
        state->status = 0;
...
int ICACHE_FLASH_ATTR Z80Emulate (Z80_STATE *state, int number_cycles)
{
        int     opcode;
...
static int ICACHE_FLASH_ATTR emulate (Z80_STATE * state, int number_cycles, int opcode)

{
		...
		if (pc==0x347) {load_prog(); pc=0x207; Z80_FETCH_BYTE(pc, opcode);}
